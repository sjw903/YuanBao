...
public class PwdEditText extends AppCompatEditText {
    ...
    private void setAttrs(AttributeSet attrs) {
        ...
        if (t != null) {
            ...
            spzceX = t.getDimensionPixelSize(R.styleable.PwdEditText_space,
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3,
                            getResources().getDisplayMetrics()));
            spzceY = t.getDimensionPixelSize(R.styleable.PwdEditText_space,
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3,
                            getResources().getDisplayMetrics()));
            StrokeWidth = t.getDimensionPixelSize(R.styleable.PwdEditText_strokeWidth,
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1,
                            getResources().getDisplayMetrics()));
            Round = t.getDimensionPixelSize(R.styleable.PwdEditText_round,
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3,
                            getResources().getDisplayMetrics()));
            Circle = t.getDimensionPixelSize(R.styleable.PwdEditText_circle,
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 7,
                            getResources().getDisplayMetrics()));
            textSize = t.getDimensionPixelSize(R.styleable.PwdEditText_textSize,
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 16,
                            getResources().getDisplayMetrics()));
            checkedColor = t.getColor(R.styleable.PwdEditText_checkedColor,
                    context.getResources().getColor(R.color.colorE4E4E4));
            defaultColor = t.getColor(R.styleable.PwdEditText_defaultColor,
                    context.getResources().getColor(R.color.colorE4E4E4));
            backColor = t.getColor(R.styleable.PwdEditText_backColor,
                    context.getResources().getColor(R.color.colorFFFFFF));
            textColor = t.getColor(R.styleable.PwdEditText_textColor,
                    context.getResources().getColor(R.color.colorAAAAAA));
            waitInputColor = t.getColor(R.styleable.PwdEditText_waitInputColor,
                    context.getResources().getColor(R.color.colorE4E4E4));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < textLength; i++) {
            ...
            //RectF的参数(left,  top,  right,  bottom);
            // 画出每个矩形框并设置间距，间距其实是增加左边框距离，缩小上下右边框距离；
            RectF rect = new RectF(i * Wide + spzceX, spzceY,
                    i * Wide + Wide - spzceX, Wide - spzceY);
            ...
            if (isWaitInput && i == mText.length()) {  //显示待输入的线
                ...
                canvas.drawLine(i * Wide + Wide / 2, Wide / 2 - Wide / 5,
                        i * Wide + Wide / 2, Wide / 2 + Wide / 5, l);
            }
        }
        //画密码圆点
        for (int j = 0; j < mText.length(); j++) {
            if (isPwd) {
                ...
                canvas.drawText(mText.substring(j, j + 1),
                        rectFS.get(j).centerX() - (textSize - spzceX) / 2,
                        rectFS.get(j).centerY() + (textSize - spzceY) / 2, textPaint);
            }
        }
    }

...
public class UGCKitVideoCut extends AbsVideoCutUI implements PlayerManagerKit.OnPreviewListener {
    ...
    public void setProgressText(int tag) {
        if (tag == 1) {
            ...
            mProgressFragmentUtil = new ProgressFragmentUtil((FragmentActivity) getContext(), getResources().getString(R.string.ugckit_cover_cutting));
        }
    }
    ...
    public void setVideoPath(final String videoPath) {
        ...
        if (VideoEditerSDK.getInstance().getTXVideoInfo() != null) {
            ...
            Log.e("========短视频====裁剪：：：", width + "===width====height===" + height);
            // 初始化播放器界面[必须在setPictureList/setVideoPath设置数据源之后]
            getVideoPlayLayout().initPlayerLayout(height > width ? true : false);
        } else {
    ...
    private void loadVideoInfo(String videoPath) {
        // 加载视频信息
        TXVideoEditConstants.TXVideoInfo info = TXVideoInfoReader.getInstance(ApplicationConfigHelper.mApplication).getVideoFileInfo(videoPath);
        if (info == null) {

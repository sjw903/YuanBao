...
public class TCMusicDownloadProgress {
    ...
    public void start(@Nullable Downloadlistener listener) {
        ...
        File onlineMaterialDir = VideoDeviceUtil.getExternalFilesDir(ApplicationConfigHelper.mApplication, BGM_FOLDER);
        if (onlineMaterialDir == null || onlineMaterialDir.getName().startsWith("null")) {
            mListener.onDownloadFail(ApplicationConfigHelper.mApplication.getResources().getString(R.string.ugckit_bgm_download_progress_no_enough_storage_space));
            stop();
            return;
        }

...
public class VideoGenerateKit extends BaseGenerateKit implements TXVideoGenerateListener {
    ...
    public void stopGenerate() {
        ...
        if (mCurrentState == PlayState.STATE_GENERATE) {
            ToastUtil.toastShortMessage(ApplicationConfigHelper.mApplication.getResources().getString(R.string.ugckit_video_editer_activity_cancel_video_generation));
    ...
    public void addTailWaterMark() {
        ...
        Bitmap tailWaterMarkBitmap = BitmapFactory.decodeResource(ApplicationConfigHelper.mApplication.getResources(), R.drawable.ugckit_tcloud_logo);
    ...
    private void saveAndUpdate(@NonNull TXVideoEditConstants.TXGenerateResult result) {
        if (mSaveToDCIM) {
            ...
            AlbumSaver.getInstance(ApplicationConfigHelper.mApplication).setOutputProfile(mVideoOutputPath, duration, mCoverPath);
            AlbumSaver.getInstance(ApplicationConfigHelper.mApplication).saveVideoToDCIM();
        }
        // UI更新
        if (mOnUpdateUIListener != null) {
